...
export function useSpeech({ onTranscript, onError } = {}) {
  ...
  /**
   * iOS Safari (and some other mobile browsers) only allow audio/speech
   * playback that's tightly tied to a user gesture, and can silently
   * block it once any `await` has happened first — which both playAudio()
   * and speakText() do, since they run after the /api/chat fetch resolves.
   * Call this SYNCHRONOUSLY, as the very first line inside a click/tap
   * handler (before any `await`), to "unlock" both the <audio> element and
   * SpeechSynthesis for the rest of the session. Safe to call every time;
   * it's a no-op after the first successful unlock on a given page load.
   */
  const primeAudioPlayback = useCallback(() => {
    try {
      ...
      // Best-effort — if this fails, playback will just fall back to
      // whatever the browser's default gesture policy allows.
    }
